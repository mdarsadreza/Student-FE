...
<template>
	<div>
		<label class="block text-lg text-primary-dark dark:text-primary-light mb-2" :for="textareaIdentifier">{{ label
			}}</label>
		<textarea
			...
			v-bind="$attrs" @input="$emit('update:val', $event.target.value)" cols="14" rows="6"></textarea>
	</div>
</template>
